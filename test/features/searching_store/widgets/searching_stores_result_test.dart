...
void main() {
  ...
  final String mockSearchString = 'text';
  ...
  testWidgets('Verify filterd store UI', (widgetTester) async {
    Widget widget = SearchingStoreResult(searchString: mockSearchString);
    ...
    expect(find.text('Tìm kiếm phù hợp'), findsOneWidget);
    expect(find.byType(FilteredStoreBody), findsOneWidget);
  });
  ...
  testWidgets(
      'Verify filterd store will call getFilteredStores() method cubit with searching every build',
      (widgetTester) async {
    Widget widget = SearchingStoreResult(searchString: mockSearchString);
    ...
    verify(mockCubit.getFilteredStores(mockSearchString)).called(1);
  });
